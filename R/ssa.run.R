...
ssa.run <- function(x0,a,nu,parms,tf,method,tau,f,epsilon,nc,hor,dtf,nd,
  ...
  timeOfNextCensus <- .t + censusInterval[obsCounter]  # Time of first data census
  timeForConsole   <- .t + consoleInterval # Time of first console output
  timeToTerminate  <- FALSE
  
  
  obsCounter <- 2 ### MAHASEN
  ...
  timeSeries <- rbind(timeSeries, matrix(nrow=numObs, ncol=(numCols)))  #### MAHASEN 
  ...
  while( (.t<tf) & (any(.x>0)) & 
    ...
    if (doCalc) {
      ...
      ### MAHASEN 
      # if (timeOfNextCensus <= .t && currentRow < length(censusInterval) ) { 
      #   timeSeries[currentRow,] <- c(.t, .x)
      #   currentRow              <- currentRow + 1
      #   timeOfNextCensus        <- censusInterval[currentRow]#.t + censusInterval
      #   
      #   
      ...
      #################################################################
      while(obsCounter <= numObs+1 && .t > censusInterval[obsCounter-1])
      {
        timeSeries[obsCounter,] <- c(censusInterval[obsCounter-1], .x)
        
        obsCounter <- obsCounter + 1
      }
      
      # Process will not be observed further
      if(.t > censusInterval[numObs])
      {
  ...
  #timeSeries <- timeSeries[!is.na(timeSeries[,1]),] ## MAHASEN
  
  
  for(i in obsCounter:(numObs+1))
  {
    timeSeries[i,] <- c(censusInterval[i-1], .x)
  }
